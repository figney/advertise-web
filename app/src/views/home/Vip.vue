<template>
  <div class="vip-page app-navbar white-view">
    <div class="fs-16 font-bold bg-primary padding margin-bottom">{{$t('VIP_CENTER','会员中心')}}</div>
    ...
    <div class="margin-lr margin-bottom flex align-center">
      ...
      >
        ...
        >
          <template #title>
            <span
              :class="[{'font-bold fs-16 fc-default ':(currentVip.level == vip.level)},{'opacity-70':(currentVip.level!=vip.level)}]"
            >
              {{vip.title}}
            </span>
          </template>
          <template #default>
            <div class="flex flex-direction border-radius overflow-hidden shadow-sm">
              ...
              <div class="padding-sm flex flex-direction padding-lr padding-tb-xs">
                <div class="flex align-center" style="padding: 0.0667rem 0">
                  <span class="fs-12 fc-secondary margin-right-sm">{{$t('PREDICT_DAY_PROFIT','预估日收益')}}:</span>
...
<!--        <template v-if="currentTimeItem.day==item.day"></template>-->

        <div class="flex flex-direction justify-end">
          <money-number class="text-right money-number" :value="item.money * amount"/>
<!--          <num-change class="text-right money-number" :value="item.money * amount"/>-->
...
<!--            <num-change :value="currentVip.task_num * amount"/>-->
<!--            <span style="margin-left:0.1rem">{{$t('N_TIMES',[''],'N次')}}</span>-->
          </div>
        </div>
...
<!--    </div>-->
    ...
    <div class="margin-lr margin-bottom-xs flex align-center justify-between padding-lr-xs">
      ...
      <money-number class="fc-accent font-bold money-number" :value="wallet.balance"/>
    </div>

    <div class="margin-lr margin-bottom-xs flex align-center justify-between fc-accent padding-lr-xs">
      <span class="fs-12">{{$t('NEED_PAY','需支付')}}</span>
      <money-number class="font-bold money-number" :value="amount * currentTimeItem.money"/>
<!--      <num-change class="font-bold money-number" :value="amount * currentTimeItem.money"/>-->
    </div>

    <div class="margin-lr margin-bottom">
      <van-button class="bg-primary font-bold fs-16 border-radius-sm" block @click="startDeposit">{{$t('BUY_NOW','立即开通')}}</van-button>
    </div>

<!--    <div class="font-bold fs-16 margin-lr margin-bottom-sm">{{$t('RULES_AND_DESC','规则与说明')}}</div>-->
    <div class="font-bold fs-16 margin-lr margin-bottom-sm" v-if="buyVipRule">{{buyVipRule.title}}</div>

    <div class="padding-sm fc-secondary bg-f1f1f1 margin-lr margin-bottom border-radius fs-12 render-html" v-html="buyVipRule.content" v-if="buyVipRule"/>
    ...
    <van-popup v-model="showBuySuccess" class="vw-80 border-radius-sm">
      <div class="padding flex flex-direction align-center position-re">
        ...
        <div class="font-bold fs-18 margin-bottom-xs text-center">{{$t('BUY_SUCCESS','开通成功')}}</div>
        <div class="fs-16 margin-bottom fc-accent text-center">{{$t('BUY_N_SUCCESS',[currentVip.title],'恭喜你成功开通N!')}}</div>
        <van-button class="bg-dark fc-fff border-radius-sm" block @click="doTask">{{$t('DO_TASK_NOW','马上接取任务赚钱')}}</van-button>
      </div>
    </van-popup>
    ...
    <van-popup v-model="showShortBalance" class="vw-80 border-radius-sm">
      <div class="padding flex flex-direction align-center position-re">
        ...
        <div class="font-bold fs-18 margin-bottom-xs text-center">{{$t('BUY_FAILED','开通失败')}}</div>
        <div class="fs-16 margin-bottom fc-accent text-center">{{$t('ACCOUNT_SHORT_BALANCE','账户可用余额不足')}}</div>
        <van-button class="bg-dark fc-fff border-radius-sm" block @click="toDeposit">{{$t('TO_DEPOSIT','去充值')}}</van-button>
      </div>
    </van-popup>
...
export default {
  ...
  methods: {
    ...
    getBuyVipContent() {
      if (!this.buyVipRule.content) {
        this.$http.post('v1/getArticle',{
          ...
          this.$store.commit('setState',{key:'buyVipRule',value:res.data.articles})
        }).catch(err=>{
    ...
    getUserVipInfo() {
      if (!this.userVipInfo.length) {
        this.$http.get('v1/userVipInfo')
            .then(res=>{
              this.$store.commit('setState',{key:'userVipInfo',value: res.data.vip})
              this.$store.commit('setState',{key:'userVipFinishedCount',value: res.data.ad_task_data || {}})
            })
            .catch(err=>{
              Toast.failed(err.data.message)
            })
