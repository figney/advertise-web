<template>
  <div class="buy-vip-page app-appbar white-view">
    ...
    <div class="padding-tb-sm flex flex-direction">
      <div class="fs-16 font-bold margin-lr margin-bottom">{{$t('SELECT_VIP_LEVEL','选择会员等级')}}</div>
    </div>
        ...
        >
          ...
          >
            ...
            >
              ...
              <money-number class="money-number" :value="vip.day_min * amount"/>
              <span class="margin-lr-xs">~</span>
              <money-number class="money-number" :value="vip.day_max * amount"/>
            </div>
            ...
            >
              ...
              <money-number class="money-number" :value="vip.day_min * 365 * amount"/>
              <span class="margin-lr-xs">~</span>
              <money-number class="money-number" :value="vip.day_max * 365 * amount"/>
            </div>
    ...
    <div class="margin-lr margin-bottom border-radius-sm padding bg-ecd8af flex align-center justify-between">
      <div class="fs-24 font-bold">
        <span>{{$t('N_TIMES',[(amount*currentVip.task_num).toString()],'N次')}}</span>
      </div>

      <money-number class="font-bold fs-24 money-number" :value="amount * currentVip.task_num_money"/>
    </div>

    <div class="flex align-center justify-between margin-lr margin-bottom">
      <div class="fs-16 font-bold">{{$t('PACKAGE_NUM','套餐数量')}}</div>
      <div class="flex align-center">
        <div class="flex align-center justify-center border-radius-xs bg-dark size-29" @click="subAmount">
        ...
        <div class="flex align-center justify-center border-radius-xs bg-dark size-29" @click="addAmount">
          <van-icon color="#ffffff" name="plus" />
        </div>
      </div>
    </div>

    <div class="margin-lr margin-bottom padding border-radius-sm bg-e8e8e8 fs-12 font-bold render-html" v-html="buyVipRule">
    </div>
...
<!--    </div>-->
    ...
    <div class="fixed-bottom shadow">
      <div class="margin-bottom-xs flex align-center justify-between padding-lr-xs">
        <span>{{$t('AVAILABLE','可用余额')}}</span>
        <money-number class="font-bold money-number" :value="wallet.balance"/>
      </div>

      <div class="margin-bottom-sm flex align-center justify-between fc-accent padding-lr-xs">
        <span>{{$t('NEED_PAY','需支付')}}</span>
        <money-number class="font-bold money-number" :value="amount * currentVip.task_num_money"/>
      </div>

      <van-button class="border-radius-sm bg-dark fc-fff font-bold" @click="startDeposit" block>{{$t('BUY_NOW','立即开通')}}</van-button>
    </div>
    ...
    <van-popup v-model="showBuySuccess" class="vw-80 border-radius-sm">
      <div class="padding flex flex-direction align-center position-re">
        ...
        <van-button class="bg-dark fc-fff border-radius-sm" block @click="$toRouter({name:'Task'})">{{$t('DO_TASK_NOW','马上接取任务赚钱')}}</van-button>
      </div>
    </van-popup>
