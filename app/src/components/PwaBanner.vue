<template>
  <div
    class="pwa-banner flex align-center zoomIn"
    v-if="show && ifPwa && showPwa"
  ...
  >
    <img
      ...
      @click="closeBanner"
    />
    <div class="padding-sm fc-fff flex-sub">
      {{ $t("INSTALL_APP", "添加到主屏幕") }}
    </div>
    <van-button
...
export default {
  ...
  computed: {
    ...mapState(["ifPwa", "showPwa"]),
  },
  mixins: [mobileApp],
  created() {
    let closeAt = localStorage.getItem("ClosePwaAt");
    if (
      closeAt &&
      !isNaN(closeAt) &&
      Date.now() - closeAt > 6 * 60 * 60 * 1000
    ) {
      localStorage.removeItem("ClosePwaAt");
    } else {
      this.$store.commit("setState", { key: "showPwa", value: false });
    }
  },
  mounted() {
  ...
  methods: {
    closeBanner() {
      ...
      setTimeout(() => {
        localStorage.setItem("ClosePwaAt", Date.now());
        this.show = false;
        this.$store.commit("setState", { key: "showPwa", value: false });
      }, 360);
    },
