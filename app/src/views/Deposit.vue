<template>
  <div class="deposit-page app-appbar">
    ...
    <div class="padding flex flex-direction">
      <div class="white-view border-radius-sm padding flex flex-direction margin-bottom">
        <div class="fs-16 font-bold margin-bottom-sm">{{$t('SELECT_PAYMENT','选择充值方式')}}</div>
        <van-radio-group v-model="currentChannel" class="margin-bottom" @change="currentChannelChange">
          <van-radio
          ...
          >
            <template #icon="props">
              <img src="../assets/images/checked.png" class="size-20" style="margin-top:-0.06rem" v-if="props.checked">
              <div class="border-radius-50 size-20" style="border:1px solid #BFBFBF" v-else/>
            </template>
            <template #default="props">
              <div :class="[{'fc-accent font-bold ':(currentChannel.id==ch.id)}]">
                {{ ch.title }}
              </div>
            </template>
          </van-radio>
...
<!--      <van-button class="bg-primary font-bold border-radius-sm margin-bottom">{{$t('HOW_TO_PAY','如何支付')}}?</van-button>-->
    ...
    >
      <div class="flex flex-direction">
        ...
        >
          <div class="fs-16 text-center font-bold white-view padding-top margin-bottom-sm">{{$t('SELECT_BANK','选择银行')}}</div>
    ...
    <van-popup position="bottom" v-model="showSonBank">
      <div class="white-view flex flex-direction position-re vh-100 vw-100">
        ...
        />
        <div class="fs-18 font-bold text-center padding-tb-lg" style="border-bottom:1px solid #EEECFA">{{$t('SELECT_BANK','选择银行')}}</div>
...
export default {
  ...
  methods: {
    depositAmountFormatter(v) {
      ...
      } else if (this.currentChannel.max_money !=0 && v>this.currentChannel.max_money) {
    ...
    getDepositInfo() {
      ...
      this.$http.get("v1/rechargeBegin")
      .then((res) => {
    ...
    bankPay(bank) {
      ...
      if (this.amount < min) {
        Toast.failed(this.$t('MIN_DEPOSIT_N',[min+this.system.default_currency],'最低充值金额为N'))
        return
      }
      if (this.amount > max && max!=0) {
        Toast.failed(this.$t('MAX_DEPOSIT_N',[max+this.system.default_currency],'最高充值金额为N'))
        return
      }
