<template>
  <div class="beginner-page-wrap">
    <div class="beginner-page">
      ...
      <div class="beginner-award flex flex-direction align-center justify-center">
        <div class="flex align-center margin-top-sm">
          <span style="margin-right:0.05rem;font-size:90%">$</span>
          <counter/>
        </div>
        <div class="fs-18 fc-accent margin-top-sm">{{$t('HAS_BEEN_CLAIMED','已被领取')}}</div>
      </div>

      <div class="register-get text-nowrap">
        <div class="flex align-center">
          <span>{{$t('N_USER_JOINED','已参与')}}</span>
          <num-change :value="joined" style="margin:0 0.1rem;font-size:0.6rem;color:#ffffff"/>
        ...
        <div class="flex-sub overflow-hidden scroll-wrap margin-bottom">
          ...
          >
            <div class="scroll-item" v-for="(item, index) in notice" :key="'beginner-notice-item'+index">
              <img :src="item.who | avatar" class="size-36 margin-right-xs avatar border-radius-50">
...
<!--                <span v-else-if="item.action=='profit'">{{item.who}} {{$t('GETTED_N_PROFIT',[item.fee+system.default_currency],'获得了N收益')}}</span>-->
                <span v-if="item.action=='award'">{{item.who}} {{$t('GETTED_N_AWARD',[item.fee+system.default_currency],'获得了{0}代言收入')}}</span>
                <span v-else-if="item.action=='profit'">{{item.who}} {{$t('GETTED_N_AWARD',[item.fee+system.default_currency],'获得了{0}代言收入')}}</span>
                <span v-else>{{item.who}} {{$t('SUCCESS_WITHDRAW_N',[system.default_currency+' '+item.fee],'成功提现了{0}')}}</span>
              </div>
            </div>
          </vue-seamless-scroll>
        </div>
