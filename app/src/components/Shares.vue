  ...
  <div class="share-com">
    <van-popup v-model="show" class="border-radius-xs">
      <div class="padding flex flex-direction vw-90">
        ...
        <div class="text-center margin-bottom">
          {{ $t("CLICK_ICON_SHARE_FRIEND", "点击下方图标分享给好友") }}
        </div>
        ...
        <van-button
          class="bg-dark fc-fff font-bold border-radius-xs"
          @click="show = false"
  ...
  data: () => {
    return {
      ...
      options: [
        ...
        {
          name: "Zalo",
          icon: require("../assets/images/zalo.png"),
          type: "za",
        },
        {
          name: "Telegram",
  ...
  methods: {
      ...
      switch (item.type) {
        ...
        case "tg":
          ...
          url = encodeURIComponent(
            `${this.getShareText()} ${this.getShareUrl(item.type)}`
          );
    ...
    getShareText() {
      ...
      try {
        text =
          this.share.contents[
            Math.floor(Math.random() * (this.share.contents.length - 1))
          ].title || "";
      } catch (e) {}
      return text;
    },
