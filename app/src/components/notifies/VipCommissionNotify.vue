<template>
    <div
        class="vip-commission-notify flex flex-direction align-center padding padding-top-xs"
    >
        <img
        ...
        />
        <div class="font-bold fs-18 margin-bottom text-center">
            <span v-if="notify.params.is_no_commission">{{
                $t("CAN_NOT_GET_COMMISSION", "无法获得佣金")
            }}</span>
            <span v-else>{{
                $t("COMMISSION_ARRIVED_NOTIFY", "佣金到账通知")
            }}</span>
        </div>
        ...
        >
            ...
            >
                <span class="fs-12">{{
                    $t(
                        "YOUR_FRIEND_BUY_VIP_N",
                        [
                            notify.params.level.toString(),
                            notify.params.buy_vip_level.toString(),
                        ],
                        "您的N级好友购买了VIP_N"
                    )
                }}</span>
                <money-number
                    :value="notify.params.buy_money"
                    class="money-number font-bold text-nowrap margin-left-xs"
                />
            </div>
            <div
                class="flex align-center justify-between padding-lr padding-tb-sm border-bottom"
            >
                <template v-if="notify.params.my_vip_level">
                    <span class="fs-12">{{
                        $t("BECAUSE_YOU_VIP_LEVEL_IS", "由于您的VIP等级是")
                    }}</span>
                ...
                <span class="fs-12" v-else>{{
                    $t("BECAUSE_YOU_DO_NOT_BUY_VIP", "由于您没有购买VIP")
                }}</span>
            </div>
            <div
                class="flex align-center justify-between padding-lr padding-tb-sm"
            >
                <span class="fs-12">{{
        ...
        >
            ...
            >
                <span class="fs-12">{{
                    $t("BECAUSE_YOU_BUY_VIP_SPEND", "由于您购买VIP总共消费")
                }}</span>
                <money-number
                    :value="notify.params.my_vip_buy_money_count"
                    class="money-number font-bold text-nowrap margin-left-xs"
                />
            </div>
            <div
        ...
        <van-button
            class="font-bold border-radius-sm bg-dark fc-fff margin-bottom-sm wrap-btn"
            block
            @click="startNow"
        >
            <span v-if="notify.params.is_no_commission">{{
            ...
            <span v-else>{{
                $t("INVITE_FRIEND_GET_MORE_AWARD", "邀请好友获取更多奖励")
            }}</span>
        </van-button>
