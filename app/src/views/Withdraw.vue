<template>
  <div class="withdraw-page flex flex-direction app-appbar">
    ...
    <template v-if="channel.length == 0 && inited">
      <div
        class="
          ...
          justify-center
        "
        style="min-height: calc(100vh - 2.04rem)"
      >
        <div
          class="
            flex-sub
        ...
        >
          ...
          />
          <span
            class="text-center fs-16 font-bold fc-secondary margin-bottom"
            >{{ $t("CAN_NOT_WITHDRAW_WAIT", "暂时无法提现，请稍候") }}</span
          >
          <van-button
      ...
      <template v-if="!submitSuccess">
        ...
        >
          ...
          >
            ...
            <template #right-icon>
              <div class="fc-accent font-400 fs-14">
                {{ $t("ALL", "全部") }}
              </div>
            </template>
          </van-field>
          ...
          >
            <van-radio
              ...
              :name="ch"
              :class="[
                'padding-sm border-radius-xs',
                { 'bg-e8e8e8': currentChannel.id == ch.id },
                { 'opacity-80': currentChannel.id != ch.id },
              ]"
            >
              <span
                :class="[
                  'flex',
                  'align-center',
                  { 'fc-accent': currentChannel.id == ch.id },
                ]"
              >
          ...
          <template v-if="!currentChannel.select_bank">
            <div class="padding-tb-sm flex align-center">
              <img
                ...
                class="size-25 margin-right-sm"
                v-if="currentChannel.cover"
              />
          ...
          <template v-else>
            <div class="margin-bottom-sm fs-16 font-bold">
              {{ $t("SELECT_BANK", "选择银行") }}
            </div>
            <div
              class="
                ...
                font-bold
              "
              @click="showSelectBank = true"
            >
              <img
                :src="currentBank.bank_cover"
                class="size-25 margin-right-sm"
                v-if="currentBank.bank_cover"
              />
          ...
          <div
            class="margin-bottom-sm flex align-center fc-888"
            v-if="calcMin != 0"
          >
            <span class="margin-right-sm">{{
              $t("MIN_WITHDRAW", "最小提现金额")
            }}</span>
            <div class="flex align-baseline font-bold">
              <money-number class="money-number" :value="calcMin" />
            </div>
          </div>
          ...
          >
            <span class="margin-right-sm">{{
              $t("MAX_WITHDRAW", "最大提现金额")
            }}</span>
            <div class="flex align-baseline font-bold">
              <money-number class="money-number" :value="calcMax" />
            </div>
          </div>

          <div class="margin-bottom-sm flex align-center fc-888">
            <span class="margin-right-sm">{{
              $t("WITHHDRAW_FEE", "提现手续费")
            }}</span>
        ...
        >
          <div
            class="border-radius-xs padding-sm flex flex-direction bg-f8ede5"
          >
            <div class="withdraw-tip fs-12">
              <span v-if="currentChannel.select_bank">{{
                currentBank.remark || currentChannel.remark
              }}</span>
    ...
    >
      <div class="padding position-re white-view border-radius box-shadow">
        ...
        <div class="flex flex-direction vh-70">
          <div
            class="fs-16 text-center font-bold white-view padding-tb-sm"
    ...
    <!-- 提现确认 -->
    <van-popup class="vw-80 border-radius padding" v-model="showWithdrawSure">
      <div class="flex flex-direction">
        <div class="fs-18 font-bold text-center margin-bottom">
          {{ $t("WITHDRAW_SURE", "提现确认") }}
        </div>
        <div class="margin-bottom-sm">
          {{
            $t(
              "WILL_DEDUCE_N_GIFT_MONEY",
              [withdrawInfo.deduct_money + " " + system.default_currency],
              "将扣除N赠送金"
            )
          }}
        </div>
        <div
          class="flex flex-direction padding-lr-sm fs-12 fc-888 margin-bottom"
        >
          <div
            class="margin-bottom-xs flex align-center justify-between"
            v-if="withdrawInfo.give_balance"
          >
            <span class="margin-right-xs"
              >{{ $t("GIFT_BALANCE", "赠送金余额") }}:</span
            >
            <span
              >-
              {{
                withdrawInfo.give_balance >= withdrawInfo.deduct_money
                  ? withdrawInfo.deduct_money
                  : withdrawInfo.give_balance
              }}
          ...
          <div
            ...
            v-if="withdrawInfo.money_bao_deduct_give_balance"
          >
            <span class="margin-right-xs"
              >{{ $t("MONEY_BAO_BALANCE", "赚钱宝余额") }}:</span
            >
            <span
              >- {{ withdrawInfo.money_bao_deduct_give_balance }}
              {{ system.default_currency }}</span
            >
          </div>
        </div>
        <div class="flex align-center">
          <van-button
            ...
            block
            plain
            @click="showWithdrawSure = false"
            >{{ $t("CANCEL", "取消") }}</van-button
          >
          <van-button
            color="#ff8900"
...
export default {
  ...
  computed: {
    allowSubmit() {
      ...
      if (this.currentChannel.select_bank) {
        ...
        if (
          this.currentBank.max_money != 0 &&
          this.amount > this.currentBank.max_money
        ) {
          return false;
        }
        if (
          this.currentBank.min_money != 0 &&
          this.amount < this.currentBank.min_money
        ) {
          return false;
        }
      } else {
        if (this.currentChannel.input_config) {
          ...
          if (
            ...
            this.amount > this.currentChannel.max_money
          ) {
            return false;
          }
          if (
            this.currentChannel.min_money != 0 &&
  ...
  methods: {
    ...
    beforeWithdraw() {
      ...
      if (!this.allowSubmit) {
        Toast.failed(
          this.$t("PLEASE_FILL_ALL_ACCOUNT_INFO", "请将收款信息填写完整")
        );
        return;
      }
    ...
    startWithdraw() {
      ...
      //Toast.loading("loading");
      this.$http
        .post("v1/putInWithdraw", this.currentWithdrawParams)
        .then((res) => {
          //Toast.hide();
