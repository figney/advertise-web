<template>
  <div class="task-detail-page app-appbar flex flex-direction">
    ...
    <div class="flex flex-direction" v-else>
      <div class="padding-lr-sm padding-top white-view flex flex-direction margin-bottom-sm">
        <div class="border-radius-sm padding-tb-sm padding-lr bg-dark flex flex-direction fc-fff margin-bottom-sm" id="guide-task-detail-step-1">
          <div class="flex align-center margin-bottom-sm">
            <img :src="task.icon" class="size-32 margin-right-sm">
            <span class="fs-14 font-bold flex-sub margin-right-sm">{{task.data.title}}</span>
            <div class="vip-chip">
              <template v-if="task.vip_level">
                <img src="../assets/images/icon_vip@2x.png" style="height:0.5rem">
                <span style="padding-bottom:0.04rem;margin-left:-0.06rem">{{task.vip_level}}</span>
              </template>
...
<!--              <money-number class="money-number" :value="task.money"/>-->
          ...
          <template v-else>
            <div class="fs-20 fc-accent font-bold text-center padding-xs margin-bottom-xs">{{$t('TASK_IS_ENDED','任务已结束')}}</div>
          </template>

        </div>

        <template v-if="task.data.describe">
          <div class="font-bold margin-bottom-xs">{{$t('TASK_DESC','任务描述')}}</div>
          <div class="margin-bottom fs-12 fc-secondary">{{task.data.describe}}</div>
        </template>
      </div>

      <div class="white-view padding-sm flex flex-direction">
        <template v-if="task.rest_count || user_ad_task.status=='InProgress'">
          <div class="font-bold margin-bottom-xs">{{$t('TASK_REQUIREMENTS','任务要求')}}</div>
          <div class="border-radius-sm padding-sm bg-f1f1f1 fs-12 fc-error flex align-center margin-bottom">
            <img src="../assets/images/icon_26@2x.png" style="height:0.46rem" class="margin-right-sm">
            <span>{{$t('TASK_REQUIREMENTS_CONTENT_N_CLICK',[task.complete_click_number],'分享完成之后N个有效好友点击链接即可完成任务，奖励将自动发放到您的账户')}}</span>
          </div>
          <div class="font-bold margin-bottom-xs">{{$t('TASK_STEPS','任务步骤')}}</div>

          <!-- 步骤1 -->
          <div :class="['task-step-item',{'task-step-item__active':(activeStep==1)}]" id="task-step-1">
            <div class="font-bold flex align-start margin-bottom">
              <div class="size-18 bg-dark fc-fff border-radius-50 flex align-center justify-center fs-10 margin-right-xs" style="margin-top:0.04rem">1</div>
              <span>{{$t('CLICK_RECEIVE_TASK','点击下放按钮接取任务')}}</span>
            </div>
            <van-button class="bg-primary font-bold fs-16 no-border border-radius-xs" @click="startTask" id="guide-task-detail-step-2">
              <span v-if="user_ad_task.status=='InProgress'">{{$t('HAS_RECEIVED','正在代言...')}}</span>
              <span v-else>{{$t('RECEIVE_TASK','立刻开始代言')}}</span>
            </van-button>
          </div>

          <!-- 步骤2 -->
          <div :class="['task-step-item',{'task-step-item__active':(activeStep>1)}]" id="task-step-2">
            <div class="font-bold flex align-start margin-bottom">
              <div class="size-18 bg-dark fc-fff border-radius-50 flex align-center justify-center fs-10 margin-right-xs" style="margin-top:0.04rem">2</div>
              <span>{{$t('AFTER_RECEIVE_TASK_N_HOURS',[task.valid_hour],'接受任务后请在N个小时内进行分享。保存宣传图 到手机，然后复制推广文案')}}</span>
            </div>
            <div class="margin-bottom-sm border-radius-xs flex align-center justify-center overflow-hidden" style="border:1px solid #ddd">
...
<!--            <div class="margin-bottom-sm text-center fc-accent font-bold">{{$t('LONG_TOUCH_IMG_TO_SAVE','长按此图保存宣传图片')}}</div>-->
            <div class="border-radius-sm padding-sm bg-f1f1f1 fs-12 flex align-center margin-bottom">
              <span v-if="task.data.share_content">{{task.data.share_content.replace('{URL}','')}}</span>
            </div>
            <van-button class="bg-primary font-bold fs-16 no-border border-radius-xs position-re" @click="startCopy('cp')" id="guide-task-detail-step-3">
          ...
          <div :class="['task-step-item',{'task-step-item__active':(activeStep>1)}]" id="task-step-3">
            ...
            <div class="margin-bottom-sm text-center fc-error" v-if="!showShareDetail">{{$t('CLICK_TO_QUICK_SHARE','点击图标快速分享给朋友')}}</div>
            <div class="flex align-center justify-around margin-top-sm" id="guide-task-detail-step-4">
              <div class="border-radius-sm padding-sm margin-lr-sm bg-f1f1f1">
                <img src="../assets/images/icon_17@2x.png" style="height:1.4rem" @click="copyPlat('fb')">
              </div>
              <div class="border-radius-sm padding-sm margin-lr-sm bg-f1f1f1">
          ...
          <!-- 步骤4 -->
          <div :class="['task-step-item',{'task-step-item__active':(activeStep>1)}]" id="task-step-4">
            <div class="font-bold flex align-start margin-bottom">
              <div class="size-18 bg-dark fc-fff border-radius-50 flex align-center justify-center fs-10 margin-right-xs" style="margin-top:0.04rem">4</div>
              <span>{{$t('TASK_AUTO_FINISHED_N_FRIEND',[task.complete_click_number.toString()],'分享后，有N个好友点击观看后，即可自动获取奖金。')}}</span>
            </div>

            <div class="flex flex-direction align-center justify-center margin-lr-xs" v-if="user_ad_task.status=='InProgress'">
              ...
              <div class="border-radius-sm padding-sm bg-f1f1f1 fs-12 fc-error margin-bottom">
                <span>{{$t('TIP_LET_YOUR_FRIEND_CLICK_YOUR_LINK','小提示：让您的好友快快浏览下您分享过去的内容链接，即可快速获得奖金')}}</span>
              </div>
            </div>
          </div>
        </template>
...
<!--        <img src="../assets/images/icon_10@2x.png" alt="" class="margin-bottom-sm size-52">-->
        <img :src="task.icon" class="margin-bottom-sm size-52 border-radius" style="height:1.39rem">
...
<!--        <div class="font-bold fs-18 margin-bottom-xs text-center">{{$t('UPGRADE_VIP','升级VIP')}}</div>-->
        <div class="font-bold margin-bottom-xs text-center fs-18 font-bold">
          <span class="font-400 fc-secondary fs-16 margin-right-xs">{{$t('PROMOTE_COMMISSION','奖励')}}:</span>
          <money-number class="money-number custom-unit-color" :value="task.money"/>
        </div>
        <div class="fs-16 margin-bottom fc-accent text-center">{{$t('CURRENT_TASK_ONLY_VIP_N',[task.vip_level.toString()],'升级到Lv{0}即可代言该品牌')}}</div>
        <van-button class="bg-dark fc-fff border-radius-sm" block @click="toUpgradeLevel(task.vip_level)">{{$t('UPGRADE_VIP_N_GET_MORE_MONEY',[task.vip_level.toString()],'立即获得Lv{0}资格')}}</van-button>
      </div>
    </van-popup>
    ...
    <van-popup v-model="showShortCount" class="vw-80 border-radius-sm">
      <div class="padding flex flex-direction align-center position-re">
        ...
        <img src="../assets/images/icon_10@2x.png" alt="" class="margin-bottom size-52">
        <div class="font-bold fs-18 margin-bottom-xs text-center">{{$t('TASK_COUNT_SHORT','接取任务次数不足')}}</div>
        <div class="fs-16 margin-bottom fc-accent text-center">{{$t('TASK_COUNT_SHORT_THAN_CONTINUE','接取任务次数不足，购买次数继续')}}</div>
        <van-button class="bg-dark fc-fff border-radius-sm" block @click="toBuyTaskNum">{{$t('BUY_COUNT','购买次数')}}</van-button>
      </div>
    </van-popup>
    ...
    <van-popup v-model="showTaskExpired" class="vw-80 border-radius-sm">
      <div class="padding flex flex-direction align-center position-re">
        ...
        <img src="../assets/images/icon_10@2x.png" alt="" class="margin-bottom size-52">
        <div class="font-bold fs-18 margin-bottom-xs text-center">{{$t('TASK_EXPIRED','任务超时')}}</div>
        <div class="fs-16 margin-bottom fc-accent text-center">{{$t('PLEASE_FINISH_TASK_ON_TIME','请在规定时间之内完成任务')}}</div>
        <van-button class="bg-dark fc-fff border-radius-sm" block @click="showTaskExpired=false">{{$t('RECEIVE_AGAIN','重新接取')}}</van-button>
      </div>
    </van-popup>

    <!-- 引导更多任务 -->
    <van-popup v-model="showRandomTask" class="no-bg vh-100 vw-100 flex align-center justify-center">
      <div class="padding flex flex-direction align-center position-re vw-80 border-radius-sm white-view bounceInDown">
        <van-icon
            size="0.52rem"
            name="cross"
            class="position-ab close-icon"
        ...
        <div class="fs-16 font-bold margin-bottom-sm">{{$t('DO_OTHER_GET_MORE_PROFIT','我要代言更多品牌')}}</div>

        <template v-for="(ad,i) in randomTasks">
          <ad-card :ad="ad" :key="'random-ad'+i" :showButton="false" style="width:100%" class="bg-fef3ee margin-top-sm"/>
        </template>
...
export default {
  ...
  methods: {
          ...
          this.driver.defineSteps([
            {
              ...
              popover: {
                title: `${this.$t('TASK_DETAIL_GUIDE_TITLE1','这里是代言的任务次数和奖励明细')}`,
                description: '',
            ...
            },
            {
              ...
              popover: {
                title: `${this.$t('TASK_DETAIL_GUIDE_TITLE2','第一步，点击按钮即可领取代言任务')}`,
                description: '',
                showButtons: false,
            ...
            },
            {
              ...
              popover: {
                title: `${this.$t('TASK_DETAIL_GUIDE_TITLE4','第三步，将您的专属代言词通过社交媒体软件分享给您的好友 (粘贴即可).')}`,
                description: '',
                // showButtons: false,
                // className: 'custom-driver-popover',
                position: 'bottom-center'
            ...
            },
            {
              element: '#task-step-4',
              popover: {
                title: `${this.$t('TASK_DETAIL_GUIDE_TITLE5','代言是有时间限制的哦！’3个好友点击链接即可助力你成为代言形象大使，免费领取你的代言奖励')}`,
                description: '',
                position: 'top-center'
              },
            }
    ...
    startTask() {
      ...
      Toast.loading('loading')
      this.$http.post('v1/takeTheAdTask',{
        id: this.task.id
      }).then(res=> {
        Toast.hide()
    ...
    getShareUrl(type='cp') {
      ...
      url = `${url}${url.includes('?') ? '&go_router=task_share' : '?go_router=task_share'}`
