  ...
  <div class="user-page flex flex-direction">
    ...
    <div class="flex flex-direction bg-primary">
      <div class="flex align-stretch padding-lr padding-tb-sm margin-bottom-xs">
        <img :src="user.name | avatar" class="size-52 margin-right-sm border-radius-50">

        <div class="flex flex-direction justify-between" style="padding:0.04rem 0">
...
<!--            <img :src="user.vip.vip_info.icon" class="margin-left-xs" style="height:0.5rem" v-if="user.vip"/>-->
          ...
          <div class="fs-12 fc-secondary flex align-center bg-fef3ee padding-lr-sm border-radius-xs" style="width:fit-content;padding:0.06rem 0.2667rem;margin-left:-0.08rem" @click="$copyText(user.id)">
            <span>ID: {{user.id}}</span>
            <img class="margin-left-sm" src="../../assets/images/copy_w@2x.png" style="height:0.42rem"/>
          </div>
...
<!--          </van-button>-->
    ...
    <div class="white-view border-radius padding-lr padding-tb-sm margin-lr margin-bottom flex flex-direction shadow" style="margin-top:-2rem">
      <div class="flex align-center justify-between margin-bottom">
        <span class="fs-16 font-bold">
          {{$t('MY_WALLET','我的钱包')}}
        </span>
        <div class="fc-secondary flex align-center" @click="$toRouter({name:'WalletHistory'})">
          <img src="../../assets/images/icon_4@2x.png" class="margin-right-xs" style="height:0.28rem">
      ...
      <div class="flex align-stretch justify-between">
        ...
        >
          <div class="fs-19 margin-bottom-xs text-nowrap">
            <num-change class="font-bold money-number" :value="wallet.wallet_count.balance_earnings"/>
<!--            <money-number class="font-bold money-number" :value="wallet.wallet_count.balance_earnings"/>-->
    ...
    <div class="white-view border-radius flex flex-direction margin-lr margin-bottom padding-lr padding-tb-sm shadow">
      <div class="flex align-center justify-between margin-bottom">
        <span class="fs-16 font-bold">
          {{$t('MY_TASKS','我的任务')}}
        </span>
        <div class="fc-secondary flex align-center" @click="$toRouter({name:'MyTask'})">
          {{$t('ALL_TASKS','全部任务')}}
          <van-icon name="arrow" size="15"/>
        </div>
      </div>
      <div class="flex align-center justify-between">
        ...
        >
          ...
          <span class="margin-top-xs fc-secondary">{{$t('INPROGRESS','进行中')}}</span>
        </div>
<!--        <div-->
<!--            class="flex flex-direction align-center justify-center flex-sub padding-top-xs"-->
<!--            style="width:30%"-->
...
<!--        </div>-->
        ...
        >
          ...
          <span class="margin-top-xs fc-secondary">{{$t('FINISHED','已完成')}}</span>
        </div>
      </div>
    </div>
    ...
    <div class="white-view shadow margin-bottom">
      <div class="padding-top padding-lr padding-bottom-xs">
        <div class="border-radius my-invite-card overflow-hidden">
          <div class="flex-sub flex flex-direction align-end padding-lr padding-top-sm padding-bottom">
            <div class="fs-18 font-bold fc-fff margin-bottom-sm text-nowrap text-ellipsis overflow-hidden" style="max-width:8rem">{{$t('INVITE_FRIEND_TOGETHER','邀请好友一起赚')}}</div>
            <van-button class="no-bg" color="#fff" round size="small" @click="$toRouter({name:'Invite'})" style="width:fit-content">
              <div class="fc-fff padding-lr flex align-center font-bold">
                <span class="padding-left-xs">{{$t('INVITE_NOW','立即邀请')}}</span>
                <van-icon name="arrow" size="12" class="margin-left-xs"/>
              </div>
            </van-button>
          </div>
...
<!--      </van-cell>-->
      ...
      >
        <template #title>
          <span class="font-bold fs-15">{{$t('WALLET_HISTORY','收支记录')}}</span>
        </template>
      </van-cell>
      ...
      >
        <template #title>
          <span class="font-bold fs-15">{{$t('CONTACT_US','在线客服')}}</span>
        </template>
      </van-cell>
...
.user-page {
  ...
  .my-invite-card {
    ...
    background: url("../../assets/images/banner_invitation@2x.png") no-repeat center;
    background-size: 100% auto;
  }
}
</style>
