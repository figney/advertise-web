<template>
  <div class="help-page flex flex-direction">
    <div class="">
      <div
        class="margin-bottom white-view box-shadow border-radius position-re"
      ...
      <div class="fixed-contact-button">
        <van-button
          class="breath-btn bg-primary font-bold border-radius-sm"
          block
          @click="contact"
        >
          {{ $t("CONTACT_CS", "联系客服") }}
    ...
    >
      <div class="vh-80 padding flex flex-direction">
        <div class="font-bold fs-16 margin-bottom">
          {{ currentHelpItem.title }}
        </div>
        <div
          class="fc-888 render-html flex-sub padding-bottom-xl"
          style="overflow: auto"
          v-html="currentHelpItem.content"
        />
        <div class="margin-bottom">
          <van-button
            class="border-radius-xs bg-primary"
            block
            @click="contact"
            >{{
              $t("OTHER_QUESTION_CONTACT_CS", "还有其他问题咨询客服")
            }}</van-button
          >
        </div>
      </div>
