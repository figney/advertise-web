  ...
  <div class="bank-transfer-page app-appbar flex flex-direction">
    ...
    >
      ...
      >
        {{
          $t(
            "UPLOAD_TIP",
            [receiverInfo.min_money.toString() + " " + system.default_currency],
            "请按照上述信息汇款并保留汇款截图，汇款成功后在存款记录中提交审核，最小汇款金额为{0}"
          )
        }}
      </div>
      ...
      <div class="flex flex-direction margin-bottom-sm fs-12">
        <div class="margin-bottom-sm van-overflow-hidden">
    ...
    >
      ...
      >
        <span
          ...
          style="font-size: 0.46rem; font-family: monospace"
          >{{ $t("ARRIVE_AT", "到账时间") }}</span
        >
        <van-count-down
          :time="300000"
          format="mm:ss"
          style="color: #ff0000; font-family: monospace; font-size: 0.52rem"
          class="font-bold"
      ...
      <div class="margin-bottom flex flex-direction" style="width: 8.2rem">
        <van-button
          class="border-radius-sm bg-dark fc-fff margin-bottom-sm"
          block
          @click="contact"
        >
        ...
        <van-button
          class="border-radius-sm margin-bottom-sm"
          style="border: 1px solid #5d5d62"
          block
          @click="toDepositDetail"
        >
    ...
    <van-popup v-model="showDemo" get-container="body">
      ...
      >
        ...
        <div
          class="padding fs-18 font-bold text-center box-shadow"
          style="width: 100%"
        >
          {{ $t("SEE_DEMO", "查看示例") }}
        </div>
      ...
      if (this.transferor.amount < this.receiverInfo.min_money) {
        Toast.failed(
          this.$t(
            ...
            [this.receiverInfo.min_money.toString()],
            "汇款金额不能低于N"
          )
        );
        return;
