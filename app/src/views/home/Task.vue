<template>
  <div class="task-page app-appbar position-re">
    ...
    >
      <template #title>
        <div class="fc-default font-bold">
          {{ $t("TASK_CENTER", "任务中心") }}
        </div>
      </template>
      <template #right>
    ...
    >
      <van-tab :title="$t('ALL', '全部')" name="all">
        <div
          ...
          style="height: calc(100vh - 2.265rem); overflow: auto"
        >
          <van-list
...
export default {
  ...
  methods: {
    ...
    getVipTasks(level) {
      if (
        (level == "all" && this.allTasks.stop) ||
        (level != "all" && this.tasks["vip" + level].stop)
      ) {
        return;
      }
      ...
      this.$http
        ...
        .then((res) => {
          if (level == "all") {
            ...
            this.tasks["vip" + level].list = this.tasks[
              "vip" + level
